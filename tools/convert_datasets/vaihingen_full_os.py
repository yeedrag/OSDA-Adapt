...
# Set the class that will be counted as unknown to 7.
id_to_trainid = {
    0: 0, # will get ignored anyways
    1: 7, # surface
    2: 2, # building
    3: 3, # vegetation
    4: 4, # tree
    ...
    if to_label:
        ...
        for idx, class_color in enumerate(color_map):
            value_idx = np.matmul(class_color,
                                  np.array([2, 3, 4]).reshape(3, 1))
            out[flatten_v == value_idx] = id_to_trainid[idx]
    ...
    for box in boxes:
        ...
        if to_label and sample_class_stats is not None and data_type == 'train':
            ...
            for k in range(1, 7):
                ...
                n = int(np.sum(k_mask))
                if n > 0:
                    stats[k - 1] = n #! Modified, since reduce_zero
...
def main():
    ...
    mmcv.mkdir_or_exist(osp.join(out_dir, 'ann_dir_os', 'train'))
    mmcv.mkdir_or_exist(osp.join(out_dir, 'ann_dir_os', 'val'))
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            for i, src_path in enumerate(src_path_list):
                ...
                dst_dir = osp.join(out_dir, 'ann_dir_os', data_type)
