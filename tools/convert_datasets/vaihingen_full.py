...
def save_class_stats(out_dir, sample_class_stats):
    with open(osp.join(out_dir, 'sample_class_stats_full.json'), 'w') as of:
    ...
    with open(osp.join(out_dir, 'sample_class_stats_dict_full.json'), 'w') as of:
    ...
    with open(osp.join(out_dir, 'samples_with_class_full.json'), 'w') as of:
...
def main():
    ...
    mmcv.mkdir_or_exist(osp.join(out_dir, 'ann_dir_full', 'train'))
    mmcv.mkdir_or_exist(osp.join(out_dir, 'ann_dir_full', 'val'))

    zipp_list = glob.glob(os.path.join(dataset_path, 'ISPRS_semantic_labeling_Vaihingen_ground_truth_COMPLETE.zip'))
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            for i, src_path in enumerate(src_path_list):
                ...
                dst_dir = osp.join(out_dir, 'ann_dir_full', data_type)
